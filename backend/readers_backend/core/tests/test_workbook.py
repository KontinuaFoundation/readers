...
class WorkbookTestCase(APITestCase):
    ...
    def test_create_workbook(self):
        ...
        self.assertIsNotNone(workbook.pdf, "Workbook PDF file was not uploaded.")
    ...
    def test_create_workbook_with_malformed_json(self):
        ...
        response = self.client.post(
            url, malformed_body, content_type="application/json"
        )
    ...
    def test_delete_workbook(self):
        ...
        workbook = Workbook.objects.create(
            number=1, collection=collection, chapters={}, pdf=None
        )
    ...
    def test_retrieve_workbook(self):
                ...
                "covers": [
                    {
                        ...
                        "videos": [
                            {"link": "https://youtu.be/example", "title": "Test Video"}
                        ],
                        "references": [
                            {"link": "https://example.org/", "title": "Test Reference"}
                        ],
                    }
                ],
                "start_page": 3,
        ...
        workbook = Workbook.objects.create(
            collection=collection, number=1, pdf=pdf_file, chapters=chapters_data
        )
        ...
        self.assertTrue("pdf" in data, "PDF field missing from response")

        self.assertTrue("chapters" in data, "Chapters field missing from response")
        self.assertEqual(
            2,
            len(data["chapters"]),
        ...
        self.assertEqual(
            "introduction", chapter1["id"], f"Chapter id mismatch: got {chapter1['id']}"
        )
        self.assertEqual(
            1,
            chapter1["chap_num"],
        ...
        self.assertEqual(
            "kont_intro", cover1["id"], f"Cover id mismatch: got {cover1['id']}"
        )
        self.assertEqual(
            "Introduction to Kontinua",
            cover1["desc"],
            f"Cover description mismatch: got {cover1['desc']}",
        )

        video1 = cover1["videos"][0]
        self.assertTrue("link" in video1, "Video link missing from response")
        self.assertTrue("title" in video1, "Video title missing from response")

        reference1 = cover1["references"][0]
        self.assertTrue("link" in reference1, "Reference link missing from response")
        self.assertTrue("title" in reference1, "Reference title missing from response")
    ...
    def test_create_workbook_with_chapters_with_covers_with_essential_fields_missing(
        self,
    ):
        url = reverse("workbook-list")
    ...
    def test_create_workbook_with_chapters_with_covers_with_essential_fields_empty(
        self,
    ):
        url = reverse("workbook-list")
